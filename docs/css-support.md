...
| `transition` и `@keyframes` + `animation` | **готово** | функции плавности `linear`, `ease`, `ease-in`, `ease-out`, `ease-in-out`, `cubic-bezier()`; направление, число повторов, `forwards`/`backwards`/`both` |
| `grid-*`, `float`, `clear`, `filter`, `backdrop-filter`, CSS-переменные, `calc()` | позже | grid — первый кандидат |
...
| Возможность | Статус | Примечания |
|---|---|---|
| `transition: <свойство> <время> [<плавность>] [<задержка>]` | **готово** | несколько переходов через запятую; `all` покрывает все анимируемые свойства |
| `@keyframes` с `from`/`to` и процентами | **готово** | кадры проходят через каскад, поэтому единицы и `currentColor` в них работают как обычно |
| `animation: <имя> <время> [<плавность>] [<задержка>] [<повторы>] [<направление>] [<заполнение>]` | **готово** | `infinite`, `reverse`, `alternate`, `alternate-reverse`, `forwards`, `backwards`, `both` |
| Анимируемые свойства | **готово** | `opacity`, цвета (`color`, `background-color`, цвета границ), длины (размеры, отступы, смещения, радиусы, `font-size`, `letter-spacing`, `gap`), `transform`, `flex-grow`, `flex-shrink` |
...
29. Анимации и переходы идут по кадрам: значение пересчитывается раз за кадр по часам хоста, отдельного потока нет. Точность равна длине кадра, а `transition` длиной меньше кадра сработает мгновенно.
30. Событий `transitionend` и `animationend` нет. Свойств-longhand (`transition-duration`, `animation-name` и прочих) тоже нет: работают только сокращения `transition` и `animation`.
31. Одна анимация на элемент: список через запятую в `animation` не поддерживается. У `transition` список поддержан.
32. `transform` интерполируется поэлементно по матрице, а не разложением на сдвиг, поворот и масштаб. Для сдвигов, масштаба и небольших поворотов разницы не видно, для поворота более чем на 180° результат отличается от браузерного.
33. Длины разных единиц (`10px` → `50%`) не смешиваются: значение переключается на середине перехода. `steps()` в функциях плавности не поддерживается.
34. Градиент — один слой поверх `background-color` и под `background-image`. Поддержаны `linear-gradient` с углом или `to <сторона>` и `radial-gradient` с необязательным `circle`/`ellipse`. Ключевые слова размера и позиции у `radial-gradient` (`closest-side`, `at 30% 70%`) игнорируются: рисуется круг из центра до дальнего угла. Позиции точек — только в процентах, `conic-gradient` и повторяющиеся градиенты не поддерживаются.
