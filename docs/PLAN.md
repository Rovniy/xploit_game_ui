...
| 3 HTML/DOM | **готово** (2026-09-22) | `core::Atom` (интернированные строки), `RefCounted`/`RefPtr`, `IAssetLoader`/`FileAssetLoader` (доступ только внутри корня UI), собственный DOM (`Node`/`Element`/`Text`/`Comment`/`Document`/`DocumentFragment`, id-карта, dirty-биты, сериализатор), `html::LexborHtmlParser` (документы и фрагменты для `innerHTML`), `css::Selector`+`SelectorMatcher` (типы, классы, id, комбинаторы, атрибуты, структурные псевдоклассы, `:not()`), биндинги DOM для V8 (`document`, Node/Element/Text/Comment/DOMTokenList, слабые обёртки), загрузка документа с исполнением `<script>` (inline и `src`), C ABI `xgu_view_load`/`load_html`/`reload`, C# `HtmlView.Load/LoadHtml/Reload`. Проверено: native 89/89, Unity PlayMode 17/17 |
| 4 CSS/Layout | **готово** (2026-09-22) | Свой токенизатор и парсер CSS (значения, цвета, единицы, шортхенды), `RuleIndex` + каскад с учётом origin/important/специфичности/порядка, наследование, `ComputedStyle` с типизированными полями и diff для инвалидации, UA-стили, `element.style` в JS (camelCase + `setProperty`/`cssText`), текст через skparagraph (`FontManager`, `InlineContent`, white-space, text-transform), раскладка на Yoga (`LayoutBox`, `LayoutEngine`, блок = column flex, атомарные инлайны как placeholders), hit-test, `xgu_cli layout`. Проверено: native 160/160 |
| 5 GPU renderer | **готово** (2026-09-22) | `paint::Painter` (обход со stacking contexts по CSS 2.1 Appendix E: фон, фоновое изображение с cover/contain/повтором, границы со скруглением, `box-shadow`, текст, `opacity`, `transform` с `transform-origin`, клип по `overflow`, `visibility`, `z-index`), `paint::ImageCache` (PNG/JPEG/WebP через `SkCodec`, intrinsic-размеры `<img>` идут в layout), кадр как `SkPicture` в mailbox, растеризация CPU-провайдером и D3D12-провайдером, C ABI `xgu_view_repaint`, `xgu_cli render`. Исправлено по ходу: параграф перекладывался по ширине пробного прохода Yoga (текст с `text-align` уезжал вбок), анонимный inline-бокс применял box-model родителя дважды, боксы держали устаревший `ComputedStyle` после рестайла, `element.style` не принимал сокращённые свойства. Проверено: native 180/180 (включая 5 golden-PNG без текста и попиксельные проверки), Unity PlayMode 20/20 на D3D12 без ошибок debug-слоя |
| 6 Input | **готово** (2026-09-22) | Событийная модель DOM (`dom::Event`, слушатели на `Node`, диспетчер capture/target/bubble, `stopPropagation`/`stopImmediatePropagation`/`once`/`preventDefault`), биндинги `addEventListener`/`removeEventListener`/`dispatchEvent` и прототипы `Event`/`MouseEvent`/`WheelEvent`/`KeyboardEvent`/`InputEvent`/`FocusEvent`; `input::HitTester` — точное зеркало порядка отрисовки с обратным обходом, инверсией трансформаций, клипом по `overflow`, скруглениями и `pointer-events`; `input::InputRouter` — hover-цепочка с `mouseover/out/enter/leave`, `:active` на цепочке, фокус на ближайшем фокусируемом предке, синтез `click` на общем предке press/release, `dblclick` в 500 мс, колесо, клавиши, касания (первый палец зеркалится в мышь), `WindowBlur`; `dom::TextControl` — редактирование `input`/`textarea` (UTF-16 внутри, UTF-8 на границе DOM), выделение, каретка, маска пароля, placeholder, `change` по blur и Enter; отрисовка выделения и каретки; C ABI `xgu_view_send_input`/`xgu_view_set_focus`; C# `WebInput` (uGUI-указатели + клавиатура), `WebInputEvent`, `HtmlView.SendInput/SetFocus/ScreenToView`. Побочно: тег типа в обёртках V8 (событие больше нельзя принять за узел), провайдер состояний перенесён на `Document`, так что `matches(':hover')` из скрипта и каскад видят одно и то же. Проверено: native 231/231, Unity PlayMode 26/26 на D3D12 |
| 7 JS bridge | **готово** (2026-09-22) | `IBridge` + `bridge::QueueBridge` (две ограниченные очереди под мьютексом; при переполнении выбрасываются только самые старые `Emit`/`Send`, `Call` и `Reply` не теряются никогда; лимит полезной нагрузки 4 МБ); объект `Unity` в JavaScript — `emit`, `on`, `off`, `call` с промисом; аргументы ходят JSON-массивом через `v8::JSON`, так что движку не нужна библиотека JSON; C ABI `xgu_view_poll_message`, `xgu_view_send_event`, `xgu_view_reply`, `xgu_view_bridge_dropped`; C# — собственный `WebJson` (сериализация и разбор), дерево `WebValue` с `Get<T>`/`GetObject<T>`, `WebArguments`, `HtmlView.Send/On/Off/RegisterFunction/RegisterFunctionAsync`, вычитка сообщений в главном потоке. Исключение в обработчике C# отклоняет промис страницы с `{name, message, stack}`. Побочно исправлено: placeholder атомарного инлайна добавлялся в конец параграфа, из-за чего `<button>` перед текстом раскладывался после него. Сцена Sandbox показывает сценарий MVP из ТЗ. Проверено: native 252/252, Unity PlayMode 39/39 на D3D12 |
| 8 Unity API | следующий | |
| 9–10 | не начаты | |
