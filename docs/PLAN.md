...
| 2 V8 | **native готово**, Unity-проверка ожидает закрытия редактора | Реализовано: `IJavaScriptRuntime` + `js::V8Runtime` (изолят на view, `console.*` → лог → Unity Console, `Uncaught …` со стеком, необработанные rejection'ы, microtask checkpoint), `js::V8Platform` (процесс-синглтон, ICU, предзагрузка DLL, `--no-expose-wasm`), `core::RuntimeThread` (runtime-поток с тиками и inline-режимом для тестов), C ABI `xgu_tick`/`xgu_view_execute_js`/`xgu_view_set_paused`/`xgu_view_get_state`, `HtmlView.ExecuteJS`, `xgu_cli js`. Проверено: native 31/31 (13 V8-тестов), `xgu_cli js` печатает console-вывод и возвращает 1 при uncaught error. Не проверено: PlayMode-тесты `Stage2Tests` в Unity — DLL в пакете занят открытым редактором Sandbox |
| 3–10 | не начаты | |
