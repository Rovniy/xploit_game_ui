...
| CSS-парсинг | собственный токенизатор и парсер подмножества MVP (`native/src/css`) | собственный код | — |
...
| 2026-09-22 | V8 NuGet `v8-v143-x64` / `v8.redist-v143-x64` 13.0.245.25 | **успех**. `.props` задаёт `V8_COMPRESS_POINTERS`, `V8_31BIT_SMIS_ON_64BIT_ARCH`, `V8_ENABLE_SANDBOX` (+`V8_ENABLE_CHECKS` в Debug); `v8_build_config.json`: pointer compression + sandbox включены, i18n через внешний `icudtl.dat` (10 МБ), component build (clang, MSVC STL). Состав рантайма: `v8.dll` (33 МБ), `v8_libbase.dll`, `v8_libplatform.dll`, `icuuc.dll`, `third_party_icu_icui18n.dll`, `third_party_abseil-cpp_absl.dll`, `third_party_zlib.dll`, `icudtl.dat`. Debug-пакет собран с debug CRT — Debug-конфигурация обязана линковать именно его. Монолитной (`v8_monolith`) сборки для v143 на NuGet нет — используем компонентные DLL с `/DELAYLOAD` и предзагрузкой из каталога модуля (`V8Platform::preloadDlls`). Особенность V8: встроенный no-op `console` перекрывает свойства глобального шаблона — наш `console` назначается на глобальный объект после `Context::New`. Интеграция: `cmake/FetchV8Nuget.cmake` (скачивание с nuget.org, `file(ARCHIVE_EXTRACT)`), 13 V8-тестов GoogleTest, `xgu_cli js` |
| 2026-09-22 | Парсер CSS: модуль `css` lexbor против собственного | **выбран собственный парсер** (запасной вариант из риска №6). Причины: lexbor типизирует лишь ~45 свойств, а `border-radius`, `background-*`, `box-shadow`, `transform`, `gap` приходят как `_custom` — под них всё равно нужен свой разбор значений; два пути разбора дали бы двойную работу и несогласованные ошибки. Свой токенизатор (~380 строк) плюс парсер значений и шортхендов (~900 строк) обрабатывают всё единообразно и дают точные сообщения о неподдерживаемых свойствах. lexbor остаётся HTML-парсером |
| 2026-09-22 | Раскладка на Yoga 3.2.1 | Блок = column flex, инлайновое содержимое — анонимный бокс с measure-функцией поверх skparagraph, атомарные инлайны (`<img>`, inline-block) — placeholders в параграфе. Проверено на реальной странице меню: flex-центрирование, `gap`, проценты, `position: absolute` с `right/bottom`, измеренный текст. Особенность: узел Yoga с measure-функцией не может иметь детей, поэтому атомарные инлайны хранятся отдельно от списка детей Yoga и позиционируются по `getRectsForPlaceholders()` |
